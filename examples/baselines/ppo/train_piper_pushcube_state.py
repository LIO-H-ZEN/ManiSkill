#!/usr/bin/env python3
"""
State-based PPO training for PushCube-v1 with Piper robot.
Uses obs_mode="state" which provides ground-truth object poses and robot joint states
(instead of RGB camera images). State-based training is much faster and more sample-efficient.

Usage:
    python train_piper_pushcube_state.py --num_envs=4096 --total_timesteps=50000000
    
    # Resume from checkpoint:
    python train_piper_pushcube_state.py --checkpoint=runs/PushCube-v1__.../ckpt_xxx.pt --evaluate
"""
...
if __name__ == "__main__":
    ...
    # env setup - STATE MODE (no RGB images, uses ground truth object positions)
    env_kwargs = dict(
        obs_mode="state",
        render_mode="rgb_array",
        sim_backend="physx_cuda",
        robot_uids=args.robot_uids,  # Use Piper robot
    )
    ...
    print(f"Action shape: {envs.single_action_space.shape}")
    print(f"####")
