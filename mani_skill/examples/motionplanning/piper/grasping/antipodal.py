...
ANTIPODAL_PROVIDER_VERSION = "mesh_antipodal_v5_collision_surface"
...
class AntipodalConfig:
    ...
    maximum_per_contact_pair: int = 2
    tcp_contact_depth: float = 0.0
    ...
    def __post_init__(self) -> None:
        ...
        if not 1 <= self.maximum_per_contact_pair <= self.approaches_per_pair:
            raise ValueError(
                "maximum_per_contact_pair must lie within approaches_per_pair"
            )
        if self.raycast_sample_limit <= 0 or self.raycast_batch_size <= 0:
            raise ValueError("raycast budgets must be positive")
        if not 0.0 <= self.tcp_contact_depth <= 0.02:
            raise ValueError("tcp_contact_depth must be in [0.0, 0.02] m")
...
class AntipodalGraspProvider:
    ...
    def generate(
        ...
        if not geometry.collision_meshes:
            raise ValueError("antipodal provider requires collision geometry")
        sampling_mesh = trimesh.util.concatenate(
            [mesh.copy() for mesh in geometry.collision_meshes]
        )
        if not isinstance(sampling_mesh, trimesh.Trimesh) or len(sampling_mesh.faces) == 0:
            raise ValueError("collision geometry produced an empty sampling mesh")
        ...
        pairs = build_contact_pairs(samples, self.config, mesh=sampling_mesh)
        object_center = np.asarray(sampling_mesh.center_mass, dtype=np.float64)
        if object_center.shape != (3,) or not np.all(np.isfinite(object_center)):
            raise ValueError("collision mesh has an invalid center of mass")
        proposals_by_region: dict[
            tuple[int, ...],
            dict[
                int,
                list[
                    tuple[
                        tuple[float, ...], int, np.ndarray, ContactPair, float
                    ]
                ],
            ],
        ] = {}
        for pair_index, pair in enumerate(pairs):
            ...
            midpoint_to_com = float(np.linalg.norm(pair.midpoint - object_center))
            for roll_index in range(self.config.approaches_per_pair):
                ...
                # ``piper_tcp`` is defined at the fingertip/contact plane in
                # the PiPER URDF. Keep the sampled contacts at that plane by
                # default; a small positive offset remains configurable for
                # gripper variants whose usable pad begins behind the tip.
                transform[:3, 3] = (
                    pair.midpoint + approach * self.config.tcp_contact_depth
                )
                region = tuple(
                    int(value)
                ...
                score_bucket = math.floor(pair.score / 0.05)
                priority = (
                    -float(score_bucket),
                    midpoint_to_com,
                    float(approach[2]),
                ...
                proposals_by_region.setdefault(region, {}).setdefault(
                    pair_index, []
                ).append(
                    (priority, roll_index, transform, pair, midpoint_to_com)
                )

        region_sequences: dict[
            tuple[int, ...],
            list[tuple[int, int, np.ndarray, ContactPair, float]],
        ] = {}
        region_priorities = {}
        for region, by_pair in proposals_by_region.items():
            ordered_pairs = []
            for pair_index, pair_proposals in by_pair.items():
                pair_proposals.sort(key=lambda row: row[0])
                best = pair_proposals[0]
                pair = best[3]
                pair_priority = (
                    -math.floor(pair.score / 0.05),
                    best[4],
                    -pair.score,
                    pair.width,
                    pair_index,
                )
                ordered_pairs.append((pair_priority, pair_index, pair_proposals))
            ordered_pairs.sort(key=lambda row: row[0])
            region_priorities[region] = ordered_pairs[0][0]
            sequence = []
            for approach_rank in range(self.config.maximum_per_contact_pair):
                for _, pair_index, pair_proposals in ordered_pairs:
                    if approach_rank >= len(pair_proposals):
                        continue
                    (
                        _,
                        roll_index,
                    ...
                    if len(sequence) == self.config.maximum_per_contact_region:
                        break
                if len(sequence) == self.config.maximum_per_contact_region:
                    break
            region_sequences[region] = sequence

        region_order = sorted(
            region_sequences,
            key=lambda region: (*region_priorities[region], region),
        )
        proposals = []
        region_round = 0
        while True:
            ...
            for region in region_order:
                sequence = region_sequences[region]
                if region_round >= len(sequence):
                    continue
                proposals.append((region, *sequence[region_round]))
                added = True
            if not added:
                break
            region_round += 1
        ...
        for region, pair_index, roll_index, transform, pair, midpoint_to_com in proposals:
            ...
            candidate = GraspCandidate(
                ...
                metadata={
                    ...
                    "tcp_contact_depth": self.config.tcp_contact_depth,
                },
            )
