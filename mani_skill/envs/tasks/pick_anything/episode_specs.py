...
ObjectSourceName = Literal["cube", "ycb", "interndata", "robodojo"]
...
class ObjectSpec:
    ...
    def __post_init__(self) -> None:
        if self.source not in ("cube", "ycb", "interndata", "robodojo"):
        ...
        if self.source == "cube":
            ...
            if self.source in ("interndata", "robodojo") and not self.category:
                raise ValueError(f"{self.source} ObjectSpec requires category")
            if self.source == "ycb" and self.category is not None:
                raise ValueError("ycb ObjectSpec must not define category")

    @property
    def stable_id(self) -> str:
        if self.source in ("interndata", "robodojo"):
            return f"{self.source}/{self.category}/{self.object_id}"
